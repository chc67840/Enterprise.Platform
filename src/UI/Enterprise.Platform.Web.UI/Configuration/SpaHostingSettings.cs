...
/// <summary>
/// Configures how the host serves the Angular SPA. Static-root mode: serves
/// built SPA assets directly from <see cref="StaticRoot"/> via ASP.NET's
/// static-file middleware. Dev workflow uses <c>npm run watch</c> which
/// writes to <c>dist/&lt;app&gt;/browser</c>; prod deploys copy that output
/// into the project's <c>wwwroot/</c> at build time.
/// </summary>
/// <remarks>
/// <para>
/// <b>Why static-files-only.</b> The Web.UI host must be the single
/// browser-visible origin so cookies (session + XSRF) ride every request
/// without cross-origin exemptions. Serving the SPA directly (vs. proxying
/// to an Angular dev server) is prod-parity, requires no second process,
/// and removes the dev/prod behaviour drift a proxy mode would introduce.
/// </para>
/// </remarks>
public sealed class SpaHostingSettings
{
    /// <summary>Configuration section name — <c>SpaHosting</c>.</summary>
