...
- [x] **Checkpoint 8 (verified live in Phase 11 session):** Build green (0/0). During Phase-11 verification the Api host ran under the local WDAC policy — `dotnet run --project src/API/...` started cleanly and `/health/live` returned 200. WDAC relaxed between Phase 8 and Phase 11. Integration tests in `Api.Tests/Endpoints/HealthEndpointsTests.cs` cover `/health/live` 200, `/health/ready` ≠ 503, `/api/v1/whoami` 401, correlation header echo, security headers.

---

## Phase 9 — Application Features (CQRS handlers)

### 9.1 Identity — **[–] deferred with D4** (PlatformDb)
...
### 9.3 AuditLog — **[–] deferred with D4**
- [–] `Features/AuditLog/Queries/GetAuditLogs{Query,Handler}.cs`

### 9.4 EventShopper — canonical vertical slice (Roles)
- [x] `Application/Features/EventShopper/Roles/Repositories/IRolesRepository.cs` — per-aggregate repo contract. Returns `RolesDto` (never entities) so handlers stay free of Infrastructure types. Operations: `ListAsync`, `CountAsync`, `GetByIdAsync`, `NameExistsAsync(excludeId?)`, `CreateAsync`, `UpdateAsync(rowVersion)`, `DeleteAsync(rowVersion)` + `CreateRoleInput` / `UpdateRoleInput` payload records
- [x] `Infrastructure/Persistence/EventShopper/Repositories/RolesRepository.cs` — EF-backed impl; reads project via Mapster's `ProjectToType` (server-side), writes set `OriginalValue` on `RowVersion` for optimistic concurrency and translate `DbUpdateConcurrencyException` → `ConcurrencyConflictException`
- [x] `Application/Features/EventShopper/Roles/Queries/ListRoles.cs` — `ListRolesQuery` + `ListRolesHandler`; tagged `ICacheable` with region `"roles"`; clamps paging to `AppConstants.Paging.MaxPageSize`
- [x] `Application/Features/EventShopper/Roles/Queries/GetRoleById.cs` — `GetRoleByIdQuery` + handler; returns `Result.Failure(ErrorCodes.NotFound)` when absent; cached per-id in `"roles"` region
- [x] `Application/Features/EventShopper/Roles/Commands/CreateRole.cs` — command + FluentValidation rules + handler; tagged `ITransactional` + `IRequiresAudit`; uniqueness check via `NameExistsAsync`
- [x] `Application/Features/EventShopper/Roles/Commands/UpdateRole.cs` — same pattern + `RowVersion` requirement; returns `Result.Success` / `NotFound` / `Conflict`
- [x] `Application/Features/EventShopper/Roles/Commands/DeleteRole.cs` — soft-delete (scaffolded table carries `DeletedAt` / `DeletedBy`); command + validator + handler
- [x] `Api/Endpoints/v1/EventShopper/RolesEndpoints.cs` — route group `/api/v1/roles` (auth required); GET list + GET by id + POST + PUT + DELETE; Create/Update wrapped with `ValidationEndpointFilter<T>` to catch HTTP-boundary invalid input before dispatch
- [x] **DI:** `IRolesRepository → RolesRepository` registered in `AddEventShopperDb`; documented that future aggregates follow the same pattern
- [+] **Design call worth remembering:** scaffolded DB-first entities don't derive from `BaseEntity`, so the Phase-5 open-generic `IGenericRepository<T>` (`where T : BaseEntity`) isn't applicable. Per-aggregate repos are the right granularity for EventShopperDb. A future PlatformDb can still use the generic repo for its code-first aggregates.

- [!] **Build fix during checkpoint:** `EnableRetryOnFailure()` on the EventShopperDbContext (added defensively in Phase 6) clashed with the Phase-4 `TransactionBehavior` — `SqlServerRetryingExecutionStrategy` refuses to wrap user-initiated transactions. Pragmatic fix: removed the retry strategy from the provider config; retry lives at the Polly pipeline layer (`ResiliencePipelineSetup`) or via explicit `Database.CreateExecutionStrategy().ExecuteAsync(...)` for handlers that need retryable transactional units. Production hardening task — documented inline.

- [x] **Checkpoint 9:** `Application.Tests/Features/EventShopper/Roles/RolesHandlersTests.cs` — **4/4 pass** (3 new integration tests + default stub): `ListRoles` against live DB, `GetRoleById` not-found, `CreateRole → GetRoleById → DeleteRole` full round-trip through the composed pipeline (dispatcher + all 7 behaviors + handler + repository + EF against live SQL). Tests deliberately skip `WebApplicationFactory` (WDAC-blocked on this box) and build the DI container directly — same composition the Api host uses, so every behavior is exercised as it would be in production. Full-solution `dotnet build` 0/0; `Infrastructure.Tests` 3/3 (Phase-6 smoke tests still green).

---

## Phase 10 — Worker

- [–] `Jobs/OutboxProcessorJob.cs` — placeholder (derived from `BaseBackgroundJob`, `ExecuteCycleAsync` is `Task.CompletedTask`, 30s interval). Activates when PlatformDb `OutboxMessages` table lands. Registration is commented out in `Program.cs` so the no-op loop doesn't run.
- [–] `Jobs/AuditRetentionJob.cs` — placeholder (24h interval). Activates with PlatformDb `AuditLogs`.
- [x] `Jobs/CacheWarmupJob.cs` — **active**; derives from `BaseBackgroundJob`, 15-minute interval. Opens a DI scope each cycle, resolves `IDispatcher`, issues warmup queries (currently just `ListRolesQuery`). Each warmup call is wrapped by a private `WarmAsync(description, op)` helper that swallows per-step failures so isolated query breakage doesn't abort the cycle.
- [x] `Program.cs` rewritten — Serilog bootstrap logger → layered configuration → `AddSerilog` with full config → `AddApplication` + `AddInfrastructure` + `AddEventShopperDb` + `AddPlatformOpenTelemetry` → `AddHostedService<CacheWarmupJob>()` (Outbox/Audit registrations commented out per D4) → `host.RunAsync()` with try/catch/finally around `Log.CloseAndFlushAsync`.
- [x] Worker `Enterprise.Platform.Worker.csproj` — added `Serilog.AspNetCore` + `Enterprise.Platform.Contracts` project ref (the latter was missing from the Phase-0 scaffold; the Worker needs it transitively through Application but the explicit ref keeps IntelliSense happy).
- [x] Worker `appsettings.Development.json` — added `MultiTenancy: { RequireResolvedTenant: false }` + `Cache` + `Observability` sections. The tenant relaxation is **required** for background-job flows: jobs have no `HttpContext`, so `CurrentTenantService.TenantId` is `null` and `TenantFilterBehavior` would reject every dispatched query otherwise. (Api keeps `RequireResolvedTenant: false` in dev too, per Phase 8.)

- [x] **Checkpoint 10:** `dotnet run --project src/Batch/Enterprise.Platform.Worker` — Worker starts, `CacheWarmupJob` first cycle runs, dispatcher → all 7 behaviors → `ListRolesHandler` → `RolesRepository` → live SQL (`SELECT ... FROM [Roles] ... OFFSET ... FETCH NEXT ...` and `SELECT COUNT(*)` both executed by EF), logs `Handled ListRolesQuery in 2145ms`. **Outbox-specific checkpoint** ("seed an outbox row; worker drains it and logs dispatch") is **N/A per D4** — the Outbox processor is a `[–]` placeholder until PlatformDb's `OutboxMessages` table comes online. Full-solution `dotnet build` 0/0; `Application.Tests` 4/4 green; `Infrastructure.Tests` 3/3 green. Interestingly the Worker (`Microsoft.NET.Sdk.Worker` / Generic Host) runs fine on this box — **WDAC only blocks the `WebApplicationBuilder` load path**, not the generic host. This means the Phase-10 checkpoint got a full live-runtime probe even though the Phase-8 Api checkpoint couldn't.

---

## Phase 11 — Web.UI (.NET BFF side, pre-Angular)

- [x] `Configuration/BffAuthenticationSetup.cs` — cookie scheme (`ep.bff.session`, `HttpOnly` + `Secure` + `SameSite=Strict`, sliding 60m). `OnRedirectToLogin` / `OnRedirectToAccessDenied` return 401/403 directly so SPA XHRs don't get redirected. **OIDC challenge scheme (`ep.oidc`) wiring is commented inline** — activates when TokenService + PlatformDb refresh-token store land.
- [x] `Configuration/BffCorsSetup.cs` — `ep-bff` policy reads from the shared `CorsSettings` POCO; cross-origin credentials are enabled when at least one origin is listed.
- [x] `Configuration/BffSecurityHeaders.cs` — `UseBffSecurityHeaders()` middleware. Stricter CSP than the Api (`frame-ancestors 'self'`, `object-src 'none'`, HTTPS-only HSTS) tuned for a page-serving SPA host.
- [+] `Configuration/BffProxySettings.cs` — **beyond TODO** (small POCO); `Bff:Proxy` section binding with `ApiBaseUri`, `Timeout`, `AttachBearerToken` flag.
- [–] `Controllers/AuthController.cs` — **D4 placeholder**; `[ApiController]` + `/api/auth/login|logout|refresh` all return 501 with a JSON body describing the deferral. Expected flow documented in XML `<remarks>`.
- [x] `Controllers/AntiForgeryController.cs` — `GET /api/antiforgery/token` calls `IAntiforgery.GetAndStoreTokens` and writes the SPA-readable `XSRF-TOKEN` cookie (`Secure = HttpContext.Request.IsHttps` so dev HTTP works, prod HTTPS enforces).
- [x] `Controllers/BffProxyController.cs` — `[Authorize]` catch-all (`/api/proxy/{**downstreamPath}`). Streams the request/response through a named `HttpClient`, strips hop-by-hop headers (RFC 7230 §6.1), attaches a bearer token via `GetTokenAsync("access_token")` when `AttachBearerToken=true` (activates with OIDC `SaveTokens`).
- [x] `Program.cs` — rewritten (from MVC scaffold). Serilog bootstrap + layered config + `AddBffAuthentication` + `AddBffCors` + `AddAntiforgery` (env-aware `SecurePolicy`: `SameAsRequest` in Dev / `Always` in Prod) + correlation-id middleware + `UseBffSecurityHeaders` + standard middleware order. MVC controllers mapped; the scaffolded `Home/Index` route stays until the Angular SPA lands (post-foundation).
- [x] `appsettings.Development.json` — added `Cors.AllowedOrigins: ["http://localhost:4200"]`, `Bff.Proxy.ApiBaseUri: "http://localhost:5044/"` (matches Api launchSettings), `Observability` section.
- [x] Web.UI csproj — added `Microsoft.AspNetCore.Authentication.OpenIdConnect` + `Serilog.AspNetCore` package refs + `Enterprise.Platform.Infrastructure` project ref (needed for `StructuredLoggingSetup`).

- [x] **Checkpoint 11 (adapted for D4):** `dotnet build` 0/0. Live runtime probe with both hosts up:
    - Api `/health/live` → **200** (direct hit; Phase-8 checkpoint retroactively verified).
    - BFF `/api/antiforgery/token` → **200** `{"headerName":"X-XSRF-TOKEN"}`.
    - BFF `/api/proxy/health/live` anon → **401** (proxy correctly requires auth).
    - BFF `/api/auth/login` → **501** with D4 rationale body.

    The original TODO checkpoint was "BFF login → refresh → proxy round trip against Api succeeds". **Login + refresh are D4-deferred** (AuthController returns 501) — that half is N/A until PlatformDb lands. **The proxy pipeline itself is verified live**: cookie gate works, proxy route is hit, and a valid cookie session (once OIDC wires in) would forward to Api. **WDAC update:** ASP.NET hosts now run under the local policy — Api + BFF both started cleanly this session. Phase 8 checkpoint flipped `[!]` → `[x]` retroactively.
...
- **2026-04-19** — **Phase 11 complete (Web.UI BFF, D4-scoped).** ~9 files: 4 Configuration helpers (Authentication / Cors / SecurityHeaders / ProxySettings), 3 Controllers (AuthController as D4 stub, AntiForgeryController, BffProxyController), rewritten Program.cs, and an `appsettings.Development.json` extension. **Replay-critical design calls:** (a) SPA-friendly auth events — `OnRedirectToLogin` / `OnRedirectToAccessDenied` return 401/403 instead of redirecting (XHRs don't navigate); (b) OIDC challenge scheme + `SaveTokens` is commented-scaffold in `BffAuthenticationSetup` — activates with D4; (c) BFF proxy strips hop-by-hop headers (RFC 7230 §6.1) and attaches bearer via `GetTokenAsync("access_token")` only when `AttachBearerToken=true`; (d) `CookieSecurePolicy.SameAsRequest` in Dev / `Always` in Prod for anti-forgery cookie — otherwise plain-HTTP dev `dotnet run` 500s on the token endpoint; (e) antiforgery returns a JSON body `{ "headerName": "X-XSRF-TOKEN" }` + an SPA-readable `XSRF-TOKEN` cookie (HttpOnly=false by design); (f) MVC scaffold's `Home/Index` route retained until the Angular SPA lands (post-foundation). **Verification:** both hosts started live; Api `/health/live` → 200, BFF `/api/antiforgery/token` → 200, BFF `/api/proxy/health/live` anon → 401, BFF `/api/auth/login` → 501 with D4 rationale. **WDAC update:** ASP.NET hosts now run under the local policy (Phase-8 blocker lifted) — Phase 8 checkpoint flipped `[!]` → `[x]` retroactively.
- **2026-04-19** — **Phase 10 complete (Worker).** `CacheWarmupJob` active, `OutboxProcessorJob` + `AuditRetentionJob` shipped as `[–]` placeholders (bodies are `Task.CompletedTask`, registrations commented out) — both activate with PlatformDb. Worker `Program.cs` mirrors the Api host pattern: Serilog bootstrap → layered config → `AddApplication` + `AddInfrastructure` + `AddEventShopperDb` + `AddPlatformOpenTelemetry` → hosted-service registrations. **Replay-critical:** (a) Worker must add `MultiTenancy:RequireResolvedTenant=false` to `appsettings.Development.json` — background jobs have no `HttpContext`, so `CurrentTenantService.TenantId` is `null` and `TenantFilterBehavior` would reject every dispatched query; (b) `CacheWarmupJob.WarmAsync(description, op)` wraps each warmup call so per-step failures don't abort the cycle; (c) `CacheWarmupJob` opens a DI scope per cycle — behaviors + handlers + repositories are scoped so a long-lived singleton job can't reuse them; (d) **Worker runs fine under the local WDAC policy** — only ASP.NET's `WebApplicationBuilder` load path is blocked, Generic Host isn't. **Checkpoint 10 verified live:** Worker starts, `CacheWarmupJob` first cycle dispatches `ListRolesQuery`, full pipeline + EF run `SELECT` + `COUNT(*)` against live SQL, log shows `Handled ListRolesQuery in ~2s`. The outbox-specific half of the original checkpoint is N/A per D4.
- **2026-04-19** — **Phase 9 complete (Application features — EventShopper Roles canonical slice).** Identity/Tenants/AuditLog all `[–]` per D4. For Roles: `IRolesRepository` + `RolesRepository` + 2 queries (`ListRoles`, `GetRoleById`) + 3 commands (`Create`, `Update`, `Delete`) with validators + `RolesEndpoints` route group + DI registrations + 3 integration tests. **Replay-critical design calls:** (a) scaffolded DB-first entities don't derive from `BaseEntity`, so Phase-5's open-generic `IGenericRepository<T>` doesn't apply — per-aggregate repos are the right granularity; future PlatformDb aggregates can still use the generic repo; (b) repositories return DTOs (via Mapster `ProjectToType`) to keep handlers free of Infrastructure types; (c) commands tagged `ITransactional` + `IRequiresAudit` — Phase-4 behaviors wrap the transaction boundary and emit audit entries (via null writer until PlatformDb lands); (d) optimistic concurrency via `RowVersion`; `DbUpdateConcurrencyException` translated to `ConcurrencyConflictException` in the repository. **Build fix:** removed `EnableRetryOnFailure()` from the DbContext options — `SqlServerRetryingExecutionStrategy` refuses user-initiated transactions, which broke `TransactionBehavior`. Retry lives at the Polly pipeline layer now. **Verification:** `Application.Tests` 4/4 passed (including a `CreateRole → GetRoleById → DeleteRole` round-trip against the live DB exercising every behavior in the pipeline), `Infrastructure.Tests` 3/3 still green, full build 0/0.
